...
{tls_url_nice_label} is used for secure communication between the client side and server side. Simplified connection initialization is presented on diagram below. Always client initializates connection by sending "hello" message to the server like on diagram below.

[plantuml, format="png", id="relation_to_ca"]
----
@startuml

participant "Client TLS" as client
participant "Server TLS" as server

client -> server: I want to establish connection securely
server --> client: Sure, I know you, so let's do it

@enduml
----

In practice this process is more complicated and well described in the articles:
...
* Add diagram of TLS session from articles.
* Move articles to the Source of informaton section.

.todo:

* Explain relation to Certificate Authority.
