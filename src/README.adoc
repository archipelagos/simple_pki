...
From architecture point of view it acts as known internal resource, where external clients is able to connect to on demand. It should listen and be available all the time so technique like {high_availability_url_nice_label} is applied to achieve this goal. Behind it actual application is located which delivers target buissness logic.

Example is {microservic_url_nice_label} using {rest_url_nice_label} API available in public {computer_network_url_nice_label}.
...
* Write short description about VPN server.

=== VPN Client

.todo:

* Write short description about VPN client.
