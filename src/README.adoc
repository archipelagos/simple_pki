...
* Write short description what is Certificate Authority in TLS.
...
{tls_url_nice_label} is used for secure communication between the client side and server side in the {present_layer_url_nice_label}. Simplified connection establishment is presented in the diagram below.
...
This communication can differ in many ways depending on the configuration applied but this not important for understanding the general building blocks and is out of the scope of this document. The important detail is described as authentication: the client verifies the server's X.509 certificate with the certificate authority that issued it. This confirms that the server is who it says it is, and that the client is interacting with the actual owner of the domain. Here comes the Certificate Authority as important player in TLS handshake, which is essencial in secure communication establishment.
